...
public class CountTest extends SqlableTestCase {
	...
	public void testCountTableSql() {
		...
		String actual = new Select().from(MockModel.class).toCountSql();
	...
	/**
	 * Shouldn't include <i>order by</i> as it has no influence on the result of
	 * <i>count</i> and should improve performance.
	 */
	public void testCountOrderBySql() {
		final String expected = "SELECT COUNT(*) FROM MockModel WHERE intField <> ? GROUP BY intField";

		String actual = new Select().from(MockModel.class).where("intField <> ?", 0).orderBy("intField")
				.groupBy("intField").toCountSql();

		assertEquals(expected, actual);
	}

	/**
	 * Should return the same count as there are entries in the result
	 * set/table.
	 */
	...
	/**
	 * Should return the same count as there are entries in the result set if
	 * the where-clause matches several entries.
	 */
	...
	/**
	 * Should return the same count as there are entries in the result set if
	 * the where-clause matches zero entries.
	 */
	...
	/**
	 * Should return the total number of rows, even if the rows are grouped. May
	 * seem weird, just test it in an SQL explorer.
	 */
