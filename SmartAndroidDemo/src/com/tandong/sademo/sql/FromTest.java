...
import com.tandong.sa.sql.query.From;
...
public class FromTest extends SqlableTestCase {
	...
	public void testLimitOffset() {
		assertSqlEquals(SELECT_PREFIX + "LIMIT 10 OFFSET 20", from().offset(20).limit(10));
	...
	public void testWhereNoArguments() {
		...
		assertSqlEquals(SELECT_PREFIX + "WHERE Id = 1 AND Id = 2 AND Id = 5", from().where("Id = 1").where("Id = 2")
				.where("Id = 5"));
	}
	...
	public void testSingleJoin() {
		assertSqlEquals(SELECT_PREFIX + "JOIN JoinModel ON MockModel.Id = JoinModel.Id", from().join(JoinModel.class)
				.on("MockModel.Id = JoinModel.Id"));

		assertSqlEquals(SELECT_PREFIX + "AS a JOIN JoinModel AS b ON a.Id = b.Id", from().as("a").join(JoinModel.class)
				.as("b").on("a.Id = b.Id"));
	...
	public void testJoins() {
		assertSqlEquals(SELECT_PREFIX + "JOIN JoinModel ON Id JOIN JoinModel2 ON Id",
				from().join(JoinModel.class).on("Id").join(JoinModel2.class).on("Id"));
	}

	public void testJoinTypes() {
		assertSqlEquals(SELECT_PREFIX + "INNER JOIN JoinModel ON", from().innerJoin(JoinModel.class).on(""));
		assertSqlEquals(SELECT_PREFIX + "OUTER JOIN JoinModel ON", from().outerJoin(JoinModel.class).on(""));
	...
	public void testGroupByHaving() {
		...
		assertSqlEquals(SELECT_PREFIX + "GROUP BY Id HAVING Id = 1", from().having("Id = 1").groupBy("Id"));
	}

	public void testAll() {
		final String expectedSql = SELECT_PREFIX
				+ "AS a JOIN JoinModel USING (Id) WHERE Id > 5 GROUP BY Id HAVING Id < 10 LIMIT 5 OFFSET 10";

		// Try a few different orderings, shouldn't change the output
		assertSqlEquals(expectedSql, from().as("a").where("Id > 5").join(JoinModel.class).using("Id").groupBy("Id")
				.having("Id < 10").limit(5).offset(10));
		assertSqlEquals(expectedSql,
				from().offset(10).having("Id < 10").join(JoinModel.class).using("Id").limit(5).as("a").where("Id > 5")
						.groupBy("Id"));
		assertSqlEquals(expectedSql,
				from().join(JoinModel.class).using("Id").offset(10).having("Id < 10").where("Id > 5").groupBy("Id")
						.limit(5).as("a"));
	}
