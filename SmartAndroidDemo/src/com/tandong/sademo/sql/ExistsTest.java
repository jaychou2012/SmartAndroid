...
public class ExistsTest extends SqlableTestCase {
	...
	public void testCountTableSql() {
		...
		String actual = new Select().from(MockModel.class).toExistsSql();
	 ...
	 * <i>exists</i> and should improve performance.
	 */
	public void testCountOrderBySql() {
		...
		String actual = new Select().from(MockModel.class).groupBy("intField").orderBy("intField")
				.where("intField <> ?", 0).toExistsSql();

		assertEquals(expected, actual);
	}

	/**
	 * Should return {@code true} since the where-clause matches rows and thus
	 * the result set isn't empty.
	 */
	...
	/**
	 * Should return {@code false} since the where-clause matches zero rows and
